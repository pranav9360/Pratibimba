const stats = [
  { label: "Total Audits", value: "842", change: "+12.5%", color: "primary" },
  { label: "Completed", value: "712", subtext: "84.5% success", color: "green-500" },
  { label: "Failed", value: "48", subtext: "Critical issues", color: "error" },
  { label: "Escalated", value: "82", subtext: "Review required", color: "purple-500" },
  { label: "Avg. Verify Time", value: "3.2", unit: "d", change: "-0.5d vs LY", color: "blue-500" },
  { label: "Compliance %", value: "94.2%", subtext: "Target reached", color: "primary-container" },
];
    ...
    <div className="p-8 space-y-6 max-w-[1440px] mx-auto">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center bg-surface-container-low px-4 py-2 rounded-lg border border-outline-variant gap-3 text-on-surface-variant">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className={`bg-surface-container-lowest rounded-xl shadow-soft border-l-4 border-${stat.color} p-6`}
          >
            <p className="text-on-surface-variant font-label-md">{stat.label}</p>
            <p className="font-display-lg text-on-background font-black">
              {stat.value}
              {stat.unit && (
                <span className="font-headline-sm font-normal">{stat.unit}</span>
              )}
            </p>
            <div className="font-label-md mt-2">
              {stat.change && (
                <span className="text-on-primary-fixed-variant">{stat.change}</span>
              )}
              {stat.subtext && (
                <span className="text-on-surface-variant/60">{stat.subtext}</span>
              )}
            </div>
          </div>
        ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-surface-container-lowest rounded-xl shadow-soft p-6 flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <h3 className="font-headline-sm text-on-background font-extrabold">
              Monthly Audit Trends
            </h3>
            <div className="flex gap-2">
              ...
              <span className="inline-flex items-center px-2 py-1 rounded bg-primary/10 text-primary text-xs font-bold">
                Target
              </span>
            </div>
          </div>
            ...
            <svg className="w-full h-full" viewBox="0 0 800 240">
              <defs>
                <linearGradient id="grad1" x1="0%" y1="0%" x2="0%" y2="100%">
                  <stop
                    offset="0%"
                    style={{ stopColor: "rgba(64, 89, 170, 0.2)", stopOpacity: 1 }}
                  />
                  <stop
                    offset="100%"
                    style={{ stopColor: "rgba(64, 89, 170, 0)", stopOpacity: 1 }}
                  />
                </linearGradient>
              </defs>
              {/* Grid Lines */}
              ...
              {/* Area */}
              <path
                d="M0,180 L133,160 L266,120 L400,140 L533,90 L666,110 L800,70 L800,240 L0,240 Z"
                fill="url(#grad1)"
              />
              {/* Line */}
              <path
                d="M0,180 L133,160 L266,120 L400,140 L533,90 L666,110 L800,70"
                fill="none"
                stroke="#4059aa"
                strokeWidth="3"
                strokeLinecap="round"
        ...
        <div className="lg:col-span-4 bg-surface-container-lowest rounded-xl shadow-soft p-6">
          <h3 className="font-headline-sm text-on-background mb-6 font-extrabold">
            Dept. Success Rates
          </h3>
          ...
          <div className="mt-8 pt-8 border-t border-outline-variant">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-body-lg font-bold text-on-background">
                  Finance Division
                </p>
              </div>
            </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-surface-container-lowest rounded-xl shadow-soft p-6 flex flex-col items-center">
          <h3 className="font-headline-sm text-on-background w-full mb-6 text-left font-extrabold">
            Verification Split
          </h3>
          <div className="relative w-40 h-40">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-headline-md font-bold text-on-background">
                85%
              </span>
          ...
          <div className="space-y-4">
            {escalationReasons.map((item) => (
              <div key={item.reason} className="flex items-center gap-4">
                <span className="w-32 font-label-md text-on-surface-variant">
                  {item.reason}
                </span>
                ...
                <span className="w-8 font-label-md font-bold">
                  {item.percentage}%
                </span>
              </div>
            ))}
      ...
      <section className="bg-surface-container-lowest rounded-xl shadow-soft overflow-hidden">
        <div className="p-6 flex justify-between items-center border-b border-outline-variant/20">
          ...
          <button className="px-4 py-2 bg-primary text-white rounded-lg font-label-md font-bold">
            Generate New Report
          </button>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-outline-variant/10">
              {recentReports.map((report, idx) => (
                <tr
                  key={idx}
                  className="hover:bg-zebra transition-colors"
                >
                  ...
                  <td className="px-6 py-4 font-data-mono text-xs">
                    {report.date}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-4">
                      <button
                        className="p-1.5 hover:bg-surface-container rounded-lg text-secondary transition-all"
                        title="Download"
                      >
                      ...
                      <button
                        className="p-1.5 hover:bg-surface-container rounded-lg text-on-surface-variant transition-all"
                        title="Share"
                      >
                        <span className="material-symbols-outlined text-xl">
                          share
