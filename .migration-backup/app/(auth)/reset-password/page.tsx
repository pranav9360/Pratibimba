  ...
  return (
    <div className="w-full max-w-[480px] bg-white p-8 rounded-xl border border-outline-variant/20 relative overflow-hidden shadow-xl">
      ...
      <div className="mb-8 text-center">
        ...
        <h1 className="font-headline-md text-on-surface mb-1">
          Reset Your Password
        </h1>
      ...
      {/* Success Message */}
      {isSuccess && (
        <div className="mb-6 p-4 bg-surface-container-high rounded-lg border border-secondary/20 flex items-start gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
          <span className="material-symbols-outlined text-secondary">
            check_circle
          </span>
          <div>
            <p className="font-label-md text-on-secondary-container font-bold">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-1.5">
            <label
              ...
              className="block font-label-md font-semibold text-on-surface-variant"
            >
              New Password
            </label>
          ...
          <div className="space-y-2 py-1">
            <div className="flex justify-between items-center">
              <span className="font-label-md text-on-surface-variant/80 font-medium">
                Password Strength
              </span>
              <span
                className={`font-data-mono text-[10px] uppercase tracking-widest ${
            ...
            <div className="h-1.5 w-full bg-surface-container rounded-full overflow-hidden flex gap-1">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`h-full w-1/4 transition-colors duration-300 ${getStrengthColor(i)}`}
                />
              ))}
            </div>
            <ul className="text-[12px] text-on-surface-variant/70 grid grid-cols-2 gap-x-4 mt-2">
              <li
                className={`flex items-center gap-1.5 ${password.length >= 8 ? "text-secondary" : ""}`}
              >
              ...
              <li
                className={`flex items-center gap-1.5 ${/[^A-Za-z0-9]/.test(password) ? "text-secondary" : ""}`}
              >
                <span className="material-symbols-outlined text-[16px]">
                  {/[^A-Za-z0-9]/.test(password) ? "check_circle" : "circle"}
                </span>
                Special symbol
              </li>
            </ul>
          ...
          <button
            type="submit"
            className="w-full mt-6 py-3.5 bg-primary text-white font-label-md font-bold rounded-lg hover:bg-primary/90 transition-all active:scale-[0.98] flex justify-center items-center gap-2 shadow-md"
          >
            Reset Password
          </button>
        </form>
      ...
      <div className="mt-8 pt-6 border-t border-outline-variant/30 flex justify-center">
        <Link
          href="/login"
          className="text-primary font-label-md font-semibold flex items-center gap-1 hover:gap-2 transition-all"
        >
